...
"""
class ImageLabeler(BasicImageWidget):
    "Displays images for manual inspection and relabelling."
    def __init__(self, dataset, classes, batch_size=5, rows=1):
        super().__init__(dataset, batch_size, classes=classes, labels=None, rows=rows)
        self.labels = {}
    ...
    def get_widgets(self, batch):
        ...
        for i, img in enumerate(batch):
            ...
            dropdown = self.make_dropdown_widget(options=self.classes, value=img.label,
                                                 handler=self.set_label, img_idx=i)
            widgets.append(self.make_vertical_box((img_widget, dropdown)))
        return widgets

    def set_label(self, change):
        self.labels[self._get_change_payload(change.owner)['idx']] = change.new
        self.labels[self._get_change_payload(change.owner)['idx']] = change.old
"""
